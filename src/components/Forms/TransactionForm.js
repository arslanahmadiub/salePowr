...
const TransactionForm = props => {
    const [state, setState] = React.useState({ transactionType: "sell", delivery: "24hrs" })
    ...
    const processRequest = event => {
        ...
        */
        setMessage("Your transaction request has been sent. We will update you when the other party accepts and makes payment.")
        setDialog(!dialog);
    ...
    const onSelectionChange = option => {
        setState({ ...state, transactionType: option === "selling" ? "sell" : "buy" })
        ...
        <Grid container direction="row" spacing={2}>
            ...
            <Grid item xs={12}>
                <Grid container direction="row" spacing={5}>
                    <Grid item xs={6}>
                        <FlatSelect onSelectionChange={onSelectionChange} bg list={["Selling", "Buying"]} />
                    </Grid>
                    <Grid item xs={6}>
                        <SearchBox onChange={handleTypingInput} onSubmit={SearchForMerchant} placeholder="Search by merchant's ID" />
                    </Grid>
            ...
            <Grid item xs={12} md={6}>
                <SubTitle>{state.transactionType === "sell" ? "Seller" : "Buyer"} Info</SubTitle>
